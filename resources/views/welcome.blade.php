            ...
            <div class="alert alert-info text-center" id="ins_upd_dt"></div>
                ...
                <fieldset class="whatdidyoudo">
                    ...
                    <div class="new">
                        ...
                        {{ Form::text('timebox', '00:00', 
                        ['class' => 'form-control']) }}
        
                        {{ Form::button('am/pm', 
                        ['name' => 'ampm', 'class' => 'btn btn-primary']) }}
                        </div>
                    </div>
...
$(document).ready(function()
{
    ...
    function AjaxTest(json, mode)
    {
        ...
        {
            $('#txtInfo1').addClass('sel_col');
        } // Add selected colour to text entry area..

        $('#txtInfo1').val(splitdata[0]); // Populate textarea

        var st_str = 'Created: ';
        if (splitdata[2] == '01/01/1990 00:00:00')
        {
            st_str += 'Not Known';
        }
        else if (splitdata[2] == '')
        ...
        st_str += 'Changed: ';
        if (splitdata[3] == '01/01/1990 00:00:00')
        {
            st_str += 'Not Known';
        }
        else if (splitdata[3] == '')
        ...
        if (mode != 'initial' && iscurweeksplit[0] == 1)
        {
            $('#li_prvwkbtn button').addClass('today_col');
        }
        ...
        if (mode != 'initial' && iscurweeksplit[2] == 1)
        {
            $('#li_nxtwkbtn button').addClass('today_col');
        }

        var uk_date_split = splitdata[9].split("-");
        document.title = 'An AJAX Diary: ' 
            + uk_date_split[2] + '-' + uk_date_split[1]
    ...
    $('#txtInfo1').keypress(function(event){if(event.which > 0)
        {showhidechangedstatus('visible');}});
    ...
    function loadCal(toggle, dateSelected)
    {
        ...
        $.ajax({
            ...
            success: function (data)
            {
                if (toggle)
                {
                    $('#dtPck').html(data).toggle();
                }
                else
                {
                    $('#dtPck').hide().html(data).show();
                }
            }
        });
    }
    ...
    $(document).on('click','.quickadddiv li', function()
    {
        ...
        { // Delete
            if ($(this).html().substr(0, 2) != '<i')
            {
                $(this).html('<input type="submit" value="Confirm" class="delentry"> <input type="button" value="X">');
            }
        }
    ...
    {
        if ($('.quickadddiv').length > 0 && m == 'initial')
        {
            $('.quickadddiv').remove();
            $('.quickadd').text('+');
        }
