...
class DiaryController extends Controller
{
    ...
    public function getDay($date = null)
    {
        ...
        foreach(['otd13', 'imp14', Carbon::now()->format('Y-m-d'), $date, $date3, $prvwk, $nxtwk] as $element)
        {
            $json[] = $element;
        }
        ...
        foreach ([$mon, $tue, $wed, $thu, $fri, $sat, $sun] as $value)
        {
            ...
            if ($value->format('Y-m-d') == $date4->format('Y-m-d'))
            {
                $class = 'sel_col';
            }
    ...
    public function entryUpdate($date)
    {
        ...
        $entry2 = new \App\Entry;
        $entry->where('d', $date)->increment('numedit');
